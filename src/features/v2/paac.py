...
def extract_paac(df, lam=LAMBDA, weight=WEIGHT):
    ...
    print(
        f"Extracting PAAC features... ({20 + lam} per sequence, λ={lam}, w={weight})")
...
df_paac = df_paac.drop(columns=["sequence"])
cols = [c for c in df_paac.columns if c != "label"] + \
    ["label"]  # ← move label to end
# ← reorder columns
df_paac = df_paac[cols]
# ────────────────────────────────────────────────────────────────
...
print(
    f"AAC features         : {len(paac_aa_cols)} {'✅' if len(paac_aa_cols)==20 else '⚠️'}")
print(
    f"Order (θ) features   : {len(paac_t_cols)} {'✅' if len(paac_t_cols)==LAMBDA else '⚠️'}")
...
max_lambda = 10
# ← FIXED: use df_seq (sequence dropped from df_paac)
sample_pos = df_seq[df_seq["label"] == 1]["sequence"].tolist()
# ← FIXED: use df_seq (sequence dropped from df_paac)
sample_neg = df_seq[df_seq["label"] == 0]["sequence"].tolist()
theta_pos_means = []
theta_neg_means = []
