...
print(
    f"   Total CTDT  : {len(CTD_PROPERTIES) * len(TRANSITION_PAIRS)} features")
...
df_ctdt = df_ctdt.drop(columns=["sequence"])
cols = [c for c in df_ctdt.columns if c != "label"] + \
    ["label"]  # ← move label to end
# ← reorder columns
df_ctdt = df_ctdt[cols]
# ────────────────────────────────────────────────────────────────
...
if os.path.exists(ctdc_path):
    ...
    axes[0].bar(range(len(ctdc_std)), ctdc_std.values,
                color="#3498db", alpha=0.8)
    axes[0].set_title("CTDC — Feature Std Dev (variability)",
                      fontweight="bold")
    ...
    axes[1].bar(range(len(ctdt_std)), ctdt_std.values,
                color="#e67e22", alpha=0.8)
    axes[1].set_title("CTDT — Feature Std Dev (variability)",
                      fontweight="bold")
