...
print(
    f"   Parameters    : {sum(p.numel() for p in esm_model.parameters())/1e6:.0f}M")
...
df_esm2 = df_esm2.drop(columns=["sequence"])
cols = [c for c in df_esm2.columns if c != "label"] + \
    ["label"]  # ← move label to end
# ← reorder columns
df_esm2 = df_esm2[cols]
# ────────────────────────────────────────────────────────────────
...
print(f"      Columns : seq_id, ESM2_0...ESM2_N (embedding features), label")
df_esm2.iloc[:3, :8]
