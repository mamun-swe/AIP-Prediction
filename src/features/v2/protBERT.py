...
cols = [c for c in df_protbert.columns if c !=
        "label"] + ["label"]  # ← move label to end
# ← reorder columns
...
print(f"      Columns : seq_id, ProtBERT_0...ProtBERT_767 (768 features), label")
...
print(
    f"Mean (pos)          : {df_protbert[df_protbert['label']==1][emb_cols].values.mean():.4f}")
print(
    f"Mean (neg)          : {df_protbert[df_protbert['label']==0][emb_cols].values.mean():.4f}")
