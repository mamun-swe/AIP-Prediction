...
df_dpc = df_dpc.drop(columns=["sequence"])
cols = [c for c in df_dpc.columns if c != "label"] + \
...
ax.bar(range(len(top30)), top30["Difference"].values,
...
plt.savefig(os.path.join(
    FIGURES_DIR, "DPC_discriminative_features.png"), dpi=150)
plt.show()
print("✅ Plot saved")
...
for idx, feat in enumerate(top6):
    ...
    ax.set_title(feat.replace("DPC_", "Dipeptide: "),
...
print(f"      Columns  : seq_id, DPC_AA...DPC_YY (400 features), label")
