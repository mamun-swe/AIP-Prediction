...
FIGURE_DIR = "../../../results/figures/features/v2"
...
df_aac = df_aac.drop(columns=["sequence"])
cols = [c for c in df_aac.columns if c != "label"] + \
    ["label"]  # ← move label to end
# ← reorder columns
df_aac = df_aac[cols]
# ────────────────────────────────────────────────────────────────
...
# ← CHANGED: show all columns
print(f"   Columns : {list(df_aac.columns)}")
df_aac.head(3)
...
print(
    f"All rows sum to 1  : {df_aac[aac_cols].sum(axis=1).round(4).eq(1.0).all()}")
print(
    f"Value range        : {df_aac[aac_cols].values.min():.4f} – {df_aac[aac_cols].values.max():.4f}")
...
ax.set_title("AAC Mean Frequency: AIP vs Non-AIP",
...
sample = df_aac.sample(min(60, len(df_aac)),
...
ax.set_title("AAC Feature Heatmap (sample of 60 sequences)",
...
ax.set_title("Discriminative Power of AAC Features",
             fontsize=13, fontweight="bold")
...
plt.savefig(os.path.join(
    FIGURE_DIR, "AAC_discriminative_features.png"), dpi=150)
plt.show()
...
# ← CHANGED: updated column description
print(f"   Columns : seq_id, AAC_A ... AAC_Y, label")
print("=" * 50)
