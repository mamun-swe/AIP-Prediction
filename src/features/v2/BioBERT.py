...
df_biobert = df_biobert.drop(columns=["sequence"])
cols = [c for c in df_biobert.columns if c !=
        "label"] + ["label"]  # ← move label to end
# ← reorder columns
df_biobert = df_biobert[cols]
# ────────────────────────────────────────────────────────────────
...
print(f"      Columns : seq_id, BioBERT_0...BioBERT_767 (768 features), label")
df_biobert.iloc[:3, :8]
