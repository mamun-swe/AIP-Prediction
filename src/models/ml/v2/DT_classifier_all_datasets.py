...
# RESULTS_DIR  = "/content/drive/MyDrive/Colab Notebooks/AIP Prediction/results/decision_tree"
# FIGURES_DIR  = "/content/drive/MyDrive/Colab Notebooks/AIP Prediction/results/figures/decision_tree"
# MODELS_DIR   = "/content/drive/MyDrive/Colab Notebooks/AIP Prediction/models/decision_tree"


# LOCAL DATA PATH (ONLY for local runs, ignored in Google Colab)
FEATURE_DIR = "../../../../data/features/v2"
RESULTS_DIR = "../../../../results/models/v2/decision_tree"
FIGURES_DIR = "../../../../results/figures/models/v2/decision_tree"
MODELS_DIR = "../../../../results/models/v2/decision_tree"
...
DT_PARAMS = {
    ...
    "max_depth"    : None,         # None = fully grown; try 5,10,15 to control overfitting
    "min_samples_split" : 2,
    "min_samples_leaf"  : 1,
    "class_weight" : "balanced",   # handles 1:1.5 class imbalance
    "random_state" : 42,
}
...
print(f"   DT criterion : {DT_PARAMS['criterion']}")
print(f"   Class weight : {DT_PARAMS['class_weight']}")
...
def compute_metrics(y_true, y_pred, y_prob):
    ...
    Note: ACC = Accuracy (same metric, listed for clarity)
    """
...
all_results  = []       # metrics for all datasets
best_model   = None     # best DT model object
best_name    = ""
best_auc     = -1.0
all_probs    = {}       # store probabilities per dataset

print("=" * 65)
print("  Decision Tree — Training & Evaluation on 11 Datasets")
print("=" * 65)
...
for ds_name, csv_file in DATASETS.items():
    ...
    prob_path = os.path.join(RESULTS_DIR, f"{ds_name}_DT_probabilities.csv")
...
print("\n── Decision Tree Performance Summary (sorted by AUC) ──")
print(df_results.to_string())

# Save summary CSV
summary_path = os.path.join(RESULTS_DIR, "DT_all_results_summary.csv")
...
best_model_path  = os.path.join(MODELS_DIR, f"DT_best_model_{best_name}.joblib")
best_scaler_path = os.path.join(MODELS_DIR, f"DT_best_scaler_{best_name}.joblib")
...
ax.set_title("Decision Tree — Performance Metrics Across All 11 Datasets",
...
plt.tight_layout()
plt.savefig(os.path.join(FIGURES_DIR, "DT_metrics_heatmap.png"),
...
for i, (col, color) in enumerate(zip(heat_cols, colors)):
    ...
    bars   = ax.bar(x + offset, df_results[col].values,
                    width, label=col, color=color,
                    alpha=0.85, edgecolor="white")

ax.set_xlabel("Dataset", fontsize=12)
ax.set_ylabel("Score", fontsize=12)
ax.set_title("Decision Tree — All Metrics per Dataset",
...
plt.tight_layout()
plt.savefig(os.path.join(FIGURES_DIR, "DT_grouped_bar_chart.png"),
            dpi=150, bbox_inches="tight")
plt.show()
...
ax.set_title("Decision Tree — ROC Curves for All Datasets",
             fontsize=14, fontweight="bold")
...
plt.tight_layout()
plt.savefig(os.path.join(FIGURES_DIR, "DT_ROC_curves.png"),
            dpi=150, bbox_inches="tight")
plt.show()
...
plt.tight_layout()
plt.savefig(os.path.join(FIGURES_DIR, f"DT_confusion_matrix_{best_name}.png"),
            dpi=150, bbox_inches="tight")
plt.show()
...
ax.set_title("Decision Tree — AUC Ranking Across All Datasets",
             fontsize=14, fontweight="bold")
ax.set_xlim(0, 1.08)
...
plt.tight_layout()
plt.savefig(os.path.join(FIGURES_DIR, "DT_AUC_ranking.png"),
            dpi=150, bbox_inches="tight")
plt.show()
...
print(f"\n  Metric Notes:")
print(f"     Sensitivity = Recall = TPR (True Positive Rate)")
print(f"     Specificity = TNR (True Negative Rate)")
print(f"     ACC         = Accuracy (same metric)")
print(f"     MCC range   : -1 (worst) to +1 (best)")
print("=" * 65)